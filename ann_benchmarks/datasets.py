...
def word2bits(out_fn, path, fn):
    ...
    local_fn = fn + ".tar.gz"
    url = "http://web.stanford.edu/~maxlam/word_vectors/compressed/{}/{}.tar.gz".format(  # noqa
        path, fn
    )
    ...
    with tarfile.open(local_fn, "r:gz") as t:
        ...
        for i in range(n_words):
            X[i] = numpy.array(
                [float(z) > 0 for z in next(f).strip().split()[1:]], dtype=numpy.bool
            )
...
def lastfm(out_fn, n_dimensions, test_size=50000):
    ...
    user_factors = numpy.append(
        model.user_factors, numpy.zeros((model.user_factors.shape[0], 1)), axis=1
    )
...
DATASETS = {
    ...
    "word2bits-800-hamming": lambda out_fn: word2bits(
        out_fn, "400K", "w2b_bitlevel1_size800_vocab400K"
    ),
