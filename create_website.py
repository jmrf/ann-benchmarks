...
def build_detail_site(data, label_func, j2_env, linestyles, batch=False):
    for (name, runs) in data.items():
        ...
        output_path = "".join(
            [args.outputdir, get_algorithm_name(name, batch), ".html"]
        )
...
def build_index_site(datasets, algorithms, j2_env, file_name):
    ...
    for mode in ["batch", "non-batch"]:
        distance_measures = sorted(
            {get_distance_from_desc(e) for e in datasets[mode].keys()}
        )
        sorted_datasets = sorted(
            {get_dataset_from_desc(e) for e in datasets[mode].keys()}
        )
...
def load_all_results():
    ...
    for properties, f in results.load_all_results():
        ...
        if properties["batch_mode"]:
            idx = "batch"
        all_runs_by_algorithm[idx].setdefault(algo, {}).setdefault(algo_ds, []).append(
            ms
        )
